...
//				std::cout <<"s: " <<count << "," << ((double)count) / ((double)n) << " --- " << ((double)(10 - s))/10<< std::endl;
				...
				t.start();
				select_6_and<T,BLOCK_SIZE><<<grid,block>>>(c0,c1,c2,c3,c4,c5,n,pred,res,0);
				cutil::cudaCheckErr(cudaDeviceSynchronize(),"Error executing select_6");//synchronize
				cub::DeviceSelect::Flagged(d_temp_storage, temp_storage_bytes, d_in, res, d_out, d_num_selected_out, n);
				cutil::cudaCheckErr(cudaDeviceSynchronize(),"Error executing DeviceSelect");//synchronize
				elapsedTime=t.lap();
				std::cout << elapsedTime << " ";
				...
				t.start();
				select_6_and<T,BLOCK_SIZE><<<grid,block>>>(c0,c1,c2,c3,c4,c5,n,pred,res,1);
				cutil::cudaCheckErr(cudaDeviceSynchronize(),"Error executing select_6");//synchronize
				cub::DeviceSelect::Flagged(d_temp_storage, temp_storage_bytes, d_in, res, d_out, d_num_selected_out, n);
				cutil::cudaCheckErr(cudaDeviceSynchronize(),"Error executing DeviceSelect");//synchronize
				elapsedTime=t.lap();
...
//				std::cout <<"s: " <<count << "," << ((double)count) / ((double)n) << " --- " << ((double)(10 - s))/10<< std::endl;
				...
				t.start();
				select_7_and<T,BLOCK_SIZE><<<grid,block>>>(c0,c1,c2,c3,c4,c5,c6,n,pred,res,0);
				...
				t.start();
				select_7_and<T,BLOCK_SIZE><<<grid,block>>>(c0,c1,c2,c3,c4,c5,c6,n,pred,res,1);
				cutil::cudaCheckErr(cudaDeviceSynchronize(),"Error executing select_7");//synchronize
				cub::DeviceSelect::Flagged(d_temp_storage, temp_storage_bytes, d_in, res, d_out, d_num_selected_out, n);
				cutil::cudaCheckErr(cudaDeviceSynchronize(),"Error executing DeviceSelect");//synchronize
				elapsedTime=t.lap();
...
//				std::cout <<"s: " <<count << "," << ((double)count) / ((double)n) << " --- " << ((double)(10 - s))/10<< std::endl;
				...
				t.start();
				select_8_and<T,BLOCK_SIZE><<<grid,block>>>(c0,c1,c2,c3,c4,c5,c6,c7,n,pred,res,0);
				...
				t.start();
				select_8_and<T,BLOCK_SIZE><<<grid,block>>>(c0,c1,c2,c3,c4,c5,c6,c7,n,pred,res,1);
				cutil::cudaCheckErr(cudaDeviceSynchronize(),"Error executing select_8");//synchronize
				cub::DeviceSelect::Flagged(d_temp_storage, temp_storage_bytes, d_in, res, d_out, d_num_selected_out, n);
				cutil::cudaCheckErr(cudaDeviceSynchronize(),"Error executing DeviceSelect");//synchronize
				elapsedTime=t.lap();
